...
class Customer(models.Model):
    ...
    客户表
    """
    # choices属性 提供备选数据，在模板中显示的时候，自定解析为下拉菜单， 元组的第一个元素表示在数据库中真实的值， 第二个表示页面上的具体内容，在浏览器上会显示第二个元素
    ...
    # mark
    phone = models.CharField('手机号', max_length=32, blank=True, null=True)
...
class UserProfile(AbstractBaseUser, PermissionsMixin):
    ...
    # reqiured_filed 使用createsuperuser管理命令创建一个命令的时候， 用来提示的字段名称列表，包含User模型中所有的必须字段，但是不包含username——filed和password
